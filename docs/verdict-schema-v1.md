...
- **`openapi/cheapagent.v1.yaml`** — OpenAPI 3.1. The HTTP binding of the same contract: `POST /v1/profile`, `/v1/convert`, `/v1/validate`, `/v1/plan` (implemented surfaces; `/v1/plan` added 2026-06-12 as the one schema-1.1 endpoint, per `docs/context-plan-design.md`), `/v1/estimate`, `/v1/batch` (spec-only, `x-status: planned`).

**One contract, two transports — spec first, implementations staged.** Status update 2026-06-12: `doc2toon serve` shipped in v0.4.0 and exposes these endpoints on localhost (`src/http-handlers.ts` carries the transport-free handlers a hosted function imports verbatim); the MCP server (v0.4.0) returns the same object as structured tool results. `api.cheapagent.ai` exposes the identical contract when demand justifies hosting it. The web app, CLI `--json` (v0.3.0), MCP tools, serve, and GitHub Action all emit this same object. No surface ever re-derives a verdict: everything calls the same `buildVerdict`. This document and the spec were published ahead of the implementations deliberately — that is what made the freeze meaningful.
